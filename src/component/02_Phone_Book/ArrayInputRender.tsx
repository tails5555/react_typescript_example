    ...
    return (
        <React.Fragment>
                ...
                value.map((val, idx) => (
                    <div className="form-group" key={`input_array_${idx}`}>
                        <div className="input-group">
                            ...
                            <input className={`form-control ${error[idx] !== '' ? 'is-valid' : 'is-invalid'}`} type="text" value={val} name={name} placeholder={`${label}-${idx}`} onChange={() => onChange(idx, value)} />
                            {
                                error[idx] !== '' ?
            ...
            <div className="text-right">
                <button className="btn btn-primary" onClick={onClickCreate}>연락처 추가</button>
            </div>
        </React.Fragment>
    )
}
