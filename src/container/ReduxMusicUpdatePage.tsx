...
class ReduxMusicUpdatePage extends React.Component<Props, {}> {
    ...
    public componentDidUpdate(prevProps : Props, prevState : any){
        ...
        const { music, error, type, status } = musicState;
        if(music !== null && type === 'UPDATE'){
            alert(`음악이 수정 되었습니다.\n음악 제목 : ${music.getTitle}`);
            resetSaveMusic();
            history.push('/example/ts_redux/music_list');
        } else if(status === 204 && type ==='DELETE') {
            alert('음악이 삭제 되었습니다. 음악 목록으로 이동합니다.');
            resetSaveMusic();
            history.push('/example/ts_redux/music_list');
    ...
    public render() {
        const { musicState, genreState, publisherState, updateMusic, deleteMusic, history, location, match } = this.props;
        const { loading, music, type } = musicState;
        const loadingState = 
            (loading === true) ?
                (music !== null) ? 
                    <div className="text-center">
                        <h2><i className="fas fa-spinner fa-spin" /> 음악을 {type === 'DELETE' ? '삭제하는' : '저장하는'} 중입니다...</h2>
                    </div> : 
                    <div className="text-center">
            ...
            <div className="container" style={{ marginTop : '10px', marginBottom : '10px' }}>
                <div id="music_view" style={{ marginTop : '10px', marginBottom : '10px' }}>
                    <MusicEditView 
                        ...
                        music={musicState.music} handleCreate={null} handleUpdate={updateMusic} handleDelete={deleteMusic}
